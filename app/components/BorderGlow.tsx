...
const BorderGlow = ({
  ...
  const [touchGlowing, setTouchGlowing] = useState(false);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    if (e.pointerType === 'touch') return; // let touch auto-glow handle it
  ...
  // On touch devices (mobile), continuously rotate the glow automatically
  useEffect(() => {
    const isTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
    if (!isTouch) return;
    setTouchGlowing(true);
    let angle = 0;
    let rafId: number;
    const rotate = () => {
      angle = (angle + 0.5) % 360;
      setCursorAngle(angle);
      setEdgeProximity(0.9); // keep proximity high every frame
  ...
  const isVisible = isHovered || sweepActive || touchGlowing;
